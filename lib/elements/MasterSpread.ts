 ...
 */
export class MasterSpread extends AbstractElementClass {
  ...
  buildFile(): boolean {
    ...
    new BuilderXML({
      path: path.join(this.bookData.idmlFolder, 'MasterSpreads', `MasterSpread_${this.self}.xml`),
      content: content,
      root: root
    }).output();
    return true
  ...
  public XMLContent(): string {
    return ''; // Pour le moment (le code ci-dessous est celui copié de Spread)
